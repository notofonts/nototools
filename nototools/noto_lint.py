...
def curve_has_off_curve_extrema(curve):
    ...
    for sign in [-1, +1]:
        angles = [
            sign * math.pi if math.fabs(angle) == math.pi else angle for angle in angles
        ]
...
def out_of_box_size(curve):
    ...
    delta = max(
        ex1 - bx1, ey1 - by1, bx2 - ex2, by2 - ey2, out_of_box_size(remaining_curve)
    )
...
def check_font(
    ...
    def warn(
        test_name, category_name, message, details=True, is_error=True, check_test=True
    ):
            ...
            if not csv_flag and (not quiet or ec or (wc and not nowarn)):
                ...
                if wc and not nowarn:
                    ...
                    if not sw and wc:
                        ...
                        print(
                            "Found %s (%s hidden)."
                            % (pluralize_errmsg(wc, False), "all" if sw == wc else sw)
                        )
        ...
        def check_ul_unicode_range():
            ...
            for bucket_index in range(128):
                if difference & (1 << bucket_index):
                    ...
                    set_unset = (
                        "not be set" if bitmap & (1 << bucket_index) else "be set"
                    )
                    warn(
            ...
            if mark_glyphs and not class_def:
                ...
                for code in mark_glyphs:
                    ...
                    if glyph not in class_def:
                        ...
                        tests.checkvalue("gdef/classdef/combining_mismatch", code)
                ...
                if not lig_caret_list_coverage:
                    ...
                    if set(lig_caret_list_coverage.glyphs) - set(ligatures):
                        warn(
                            ...
                            % ", ".join(
                                sorted(
                                    set(lig_caret_list_coverage.glyphs) - set(ligatures)
                                )
                            ),
                        )
    ...
    def check_for_bidi_pairs(cmap):
        ...
        if "GSUB" in font:
            try:
                ...
                warn(
                    "bidi", "bidi", "GSUB table with no feature record", is_error=False
                )
                feature_record = []
            ...
            if not high_divisor:
                ...
                if not (low_exp - slop <= adv <= high_exp + slop):
                    ...
                    warn(
                        ...
                        "%d and %d."
                        % (codepoint, glyph_name, glyph_id, adv, low_exp, high_exp),
                        check_test=False,
                        is_error=is_error,
                    )
...
def main():
    ...
    parser.add_argument(
        "--dump_font_props", help="write font props for files", action="store_true"
    )
    parser.add_argument(
        "-nw", "--nowarn", help="suppress warning messages", action="store_true"
    )
    parser.add_argument(
        "-q",
        "--quiet",
