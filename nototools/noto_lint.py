...
def curve_between(coordinates, start_at, end_at, start_of_contour, end_of_contour):
    ...
    if end_at > start_at:
        ...
        return list(coordinates[start_at : end_of_contour + 1]) + list(coordinates[start_of_contour : end_at + 1])
...
def curve_has_off_curve_extrema(curve):
    ...
    score = 0
    for sign in [-1, +1]:
        angles = [sign * math.pi if math.fabs(angle) == math.pi else angle for angle in angles]
...
def out_of_box_size(curve):
    ...
    delta = max(ex1 - bx1, ey1 - by1, bx2 - ex2, by2 - ey2, out_of_box_size(remaining_curve))
...
def interpolate_segment(segment, amount):
    return (interpolate(segment[0][0], segment[1][0], amount), interpolate(segment[0][1], segment[1][1], amount))
...
def cut_piece_in_half(piece):
    if len(piece) == 2:
        ...
        return bezierTools.splitQuadraticAtT(piece[0], piece[1], piece[2], 0.5)


def cut_ends(piece, cut_amount):
    if len(piece) == 2:
        return (interpolate_segment(piece, cut_amount), interpolate_segment(piece, 1 - cut_amount))
    else:
        return bezierTools.splitQuadraticAtT(piece[0], piece[1], piece[2], cut_amount, 1 - cut_amount)[1]
            ...
            for second_section in [section21, section22]:
                if probably_intersect(first_section, second_section):
                    if level > _MAX_DEPTH:
                        ...
                        pairs_to_investigate.append((first_section, second_section, level + 1))


def to_float_tuples(curve):
    coord_list = []
    for coords in curve:
        coord_list.append((float(coords[0]), float(coords[1])))
...
def curves_intersect(contour_list):
        ...
        for curve in contour:
            if len(curve) == 2:
                ...
                for curve_part in basePen.decomposeQuadraticSegment(curve[1:]):
                    contour_pieces.append(to_float_tuples((last_point,) + curve_part))
    ...
    for contour_pieces in all_contours:
        for i in range(len(contour_pieces) - 1):
            adjacent_pairs.add(frozenset({contour_pieces[i], contour_pieces[i + 1]}))
        if len(contour_pieces) > 2:
            adjacent_pairs.add(frozenset({contour_pieces[-1], contour_pieces[0]}))
...
def _build_cmap_dict(filename):
    ...
    return {script: frozenset(tool_utils.parse_int_ranges(rd.ranges)) for script, rd in script_to_rowdata.items()}
...
def _get_cmap_data_for_phase(phase):
    ...
    if phase < 3:
        if not _phase_2_map:
            _phase_2_map = _build_cmap_dict('[tools]/nototools/data/noto_cmap_phase2.xml')
        return _phase_2_map
    else:
        if not _phase_3_map:
            _phase_3_map = _build_cmap_dict('[tools]/nototools/data/noto_cmap_phase3.xml')
        return _phase_3_map
...
def font_properties_from_name(file_path, phase):
    ...
    is_google = True
    vendor = (
        'Adobe' if noto_font.is_cjk else 'KhmerType' if noto_font.script in ['Khmr', 'Cham', 'Laoo'] else 'Monotype'
    )
...
def check_font(
    ...
    def warn(test_name, category_name, message, details=True, is_error=True, check_test=True):
        ...
        def print_file_name():
            ...
            if not _printed_file_name:
                ...
                print("---\nAutomatic testing for '%s', %s:" % (_cur_file_name, printable_font_versions(font)))
        ...
        if category_name is "info":
            ...
            if not csv_flag and (not quiet or ec or (wc and not nowarn)):
                ...
                if not se:
                    ...
                    print("Found %s (%s hidden)." % (pluralize_errmsg(ec), "all" if se == ec else se))
                if wc and not nowarn:
                    sw = suppressed_warn_count[0]
                    if not sw and wc:
                        print("Found %s." % pluralize_errmsg(wc, False))
                    elif wc:
                        print("Found %s (%s hidden)." % (pluralize_errmsg(wc, False), "all" if sw == wc else sw))
    ...
    def _check_unused_names():
        ...
        for i in names:
            ...
            if i >= 256:
                warn('name/unused', 'Name', 'Name table has record #%d: "%s"' % (i, names[i]), is_error=False)
        ...
        if expected:
            if not actual:
                if expected == '-':
                    ...
                    warn(test_key, keyname, "The %s is not set, but was expecting%s '%s'" % (keyname, xre, expected))
            elif expected != '-':
                if is_re:
                ...
                if not match:
                    warn(
                        test_key, keyname, "The %s is '%s' but was expecting%s '%s'" % (keyname, actual, xre, expected)
                    )
        elif actual:
    ...
    def check_name_table():
        ...
        family_to_name_info = noto_names.family_to_name_info_for_phase(noto_phase)
        name_data = noto_names.name_table_data(noto_font, family_to_name_info, noto_phase)
        if not name_data:
            warn("name/unable_to_check", "Unable to check", "No name data available for this font.")
            return
        ...
        if (0 <= int(major_version) <= 65535) and (0 <= int(minor_version) <= 65535):
            ...
            if font_revision != major_version + "." + minor_version:
                warn(
                    "name/version/match_head",
                    "Font Revision",
                    "fontRevision in 'head' table is %s, "
                    "while font version in 'name' table is %s.%s." % (font_revision, major_version, minor_version),
                )
        else:
            warn(
                "name/version/out_of_range",
                "Version",
                "Version string has numerical parts outside the range " "[0, 65535]: '%s'." % version_string,
            )
    ...
    def _get_required_chars(noto_font, noto_phase, test_key):
        ...
        try:
            ...
            warn(test_key, "Chars", "no char data for script %s in %s" % (noto_font.script, noto_font.filepath))
        return None
    ...
    def _check_needed_chars(cmap, char_filter):
        ...
        needed_chars = _get_required_chars(noto_font, noto_phase, 'cmap/script_required')
    ...
    def _check_unexpected_chars(cmap, char_filter):
        expected_chars = _get_required_chars(noto_font, noto_phase, 'cmap/unexpected')
    ...
    def check_cmap_table():
        ...
        expected_tables = [(4, 3, 1), (12, 3, 10), (14, 0, 5), (4, 0, 3), (12, 0, 4)]
        ...
        for table in cmap_table.tables:
            if (table.format, table.platformID, table.platEncID) not in expected_tables:
                warn(
                    "cmap/tables/unexpected",
                    "cmap",
                    "'cmap' has a subtable of (format=%d, platform=%d, encoding=%d), "
                    "which it shouldn't have." % (table.format, table.platformID, table.platEncID),
                )
            elif table != (12, 0, 4):
                if table.format in cmaps:
                    ...
                    if id(table.cmap) != id(cmaps[table.format]):
                        warn(
                            "cmap/tables/notaliased",
                            "cmap",
                            "'cmap' has two format %d subtables that are not" " aliases" % table.format,
                        )
                else:
                    cmaps[table.format] = table.cmap

        if 4 not in cmaps:
            warn("cmap/tables/missing", "cmap", "'cmap' does not have a format 4 subtable, but it should.")
        ...
        if 12 in cmaps:
            ...
            if tests.check('cmap/tables/format_4_subset_of_12') and 4 in cmaps:
                for char in cmaps[4]:
                    if char not in cmap:
                        warn(
                            "cmap/tables/format_4_subset_of_12",
                            "cmap",
                            "U+%04X is mapped in cmap's format 4 subtable but " "not in the format 12 one." % char,
                            check_test=False,
                        )
                        ...
                        warn(
                            ...
                            "subtable but to %s in the format 12 one." % (char, cmaps[4][char], cmap[char]),
                            check_test=False,
                        )
        else:
            cmap = cmaps[4]
        ...
        if tests.check('cmap/required'):
            required_in_all_fonts = [0x0000, 0x000D, 0x0020]  # .null  # CR  # space
            for code in required_in_all_fonts:
                if code not in cmap:
                    warn(
                        ...
                        "https://www.microsoft.com/typography/otspec/recom.htm)." % code,
                        check_test=False,
                    )
        ...
        if tests.check('cmap/private_use'):
            needed_chars = _get_required_chars(noto_font, noto_phase, 'cmap/private_use')
            ...
            if privates_in_cmap:
                warn(
                    ...
                    % (len(privates_in_cmap), printable_unicode_range(privates_in_cmap)),
                    check_test=False,
                )
        ...
        if tests.check('cmap/non_characters'):
            ...
            if non_characters_in_cmap:
                warn(
                    ...
                    "there are: %s." % printable_unicode_range(non_characters_in_cmap),
                    check_test=False,
                )

        if tests.check('cmap/disallowed_ascii') and not (
            font_props.script == "Zsye"
            or font_props.script == "Latn"
            or font_props.script == "LGC"
            or font_props.is_cjk
        ):
            ascii_letters = noto_data.ascii_letters()
            contained_letters = ascii_letters & set(cmap.keys())
            if contained_letters:
                warn(
                    "cmap/disallowed_ascii",
                    "Chars",
            ...
            for sel, varcp, _ in sorted(unicode_data.get_variant_data(cp)):
                if not sel in vs_cmap.uvsDict:
                    warn(
                        ...
                        "this selector is not in the variant table." % (cp, sel),
                        check_test=False,
                    )
                    continue
                sel_info = None
                ...
                if varcp != -1:
                    ...
                    if expected_glyphid != sel_glyphid:
                        warn(
                            ...
                            "by %04x, but its glyph is %s." % (cp, sel, sel_glyphid, varcp, expected_glyphid),
                            check_test=False,
                        )
        ...
        if tests.check('head/hhea'):
            if font_props.is_UI_metrics:
                if hhea_table.ascent > ui_ascent:
                    warn(
                        "head/hhea/ascent",
                        "Bounds",
                        "Value of ascent in 'hhea' table is %d, but should be %d." % (hhea_table.ascent, ui_ascent),
                    )
                if hhea_table.descent < ui_descent:
                    warn(
                        "head/hhea/descent",
                        "Bounds",
                        "Value of descent in 'hhea' table is %d, but should be %d." % (hhea_table.descent, ui_descent),
                    )
            ...
            if hhea_table.lineGap != 0:
                warn(
                    "head/hhea/linegap",
                    "Line Gap",
                    "Value of lineGap in 'hhea' table is %d, but should be 0." % hhea_table.lineGap,
                )
        ...
        if tests.check('head/vhea') and vhea_table:
            if vhea_table.lineGap != 0:
                warn(
                    "head/vhea/linegap",
                    "Line Gap",
                    "Value of lineGap in 'vhea' table is %d, but should be 0." % vhea_table.lineGap,
                )
        ...
        if tests.check('head/os2'):
            if os2_table.fsType != 0:
                warn(
                    "head/os2/fstype",
                    "OS/2",
                    "Value of fsType in the 'OS/2' table is 0x%04X, but should " "be 0." % os2_table.fsType,
                )
            if os2_table.sTypoAscender != hhea_table.ascent:
                warn(
                    ...
                    "but they should be equal." % (os2_table.sTypoAscender, hhea_table.ascent),
                )
            if os2_table.sTypoDescender != hhea_table.descent:
                warn(
                    ...
                    "but they should be equal." % (os2_table.sTypoDescender, hhea_table.descent),
                )
            if os2_table.sTypoLineGap != 0:
                warn(
                    "head/os2/linegap",
                    "OS/2",
                    "Value of sTypoLineGap in 'OS/2' table is %d, but " "should be 0." % os2_table.sTypoLineGap,
                )
            ...
            if os2_table.usWinAscent != hhea_table.ascent:
                warn(
                    ...
                    "but they should be equal." % (os2_table.usWinAscent, hhea_table.ascent),
                )
            if os2_table.usWinDescent != -hhea_table.descent:
                warn(
                    ...
                    "but they should be opposites." % (os2_table.usWinDescent, hhea_table.descent),
                )
            if font_props.is_google and os2_table.achVendID != 'GOOG':
                warn(
                    "head/os2/achvendid",
                    "OS/2",
                    "Value of achVendID in the 'OS/2' table is %s, " "but should be GOOG." % os2_table.achVendID,
                )
            ...
            if os2_table.fsSelection & OS2_SEL_USE_TYPO_METRICS_MASK:
                warn("head/os2/fsselection", "OS/2", "UseTypoMetrics bit in fsSelection is set, but should be clear.")

            check_ul_unicode_range()

            if os2_table.panose.bFamilyType != 2:
                warn(
                    "head/os2/panose/family",
                    "OS/2",
                    "Panose family value is %s but expected 2" % os2_table.panose.bSerifStyle,
                )

            expect_serif = noto_font.style == 'Serif' or noto_font.family in ['Cousine', 'Tinos']
            expected_serif_range_str = '2-10' if expect_serif else '11-15'
            serif_val = os2_table.panose.bSerifStyle
            is_serif = 1 < serif_val < 11
            if serif_val == 1:
                warn(
                    "head/os2/panose/serif",
                    "OS/2",
                    "Panose serif value is 1 (no_fit) but expected 0 or %s" % expected_serif_range_str,
                )
            elif serif_val != 0 and expect_serif != is_serif:
                warn(
                    "head/os2/panose/serif",
                    "OS/2",
                    "Panose serif value is %s but expected %s" % (serif_val, expected_serif_range_str),
                )
            ...
            if proportion_val <= 1:
                ...
                warn(
                    ...
                    "expected %s" % (proportion_val, 'no_fit' if proportion_val == 1 else 'any', expect_mono_range_str),
                )
            elif expect_mono != is_mono:
                warn(
                    "head/os2/panose/proportion",
                    "OS/2",
                    "Panose proportion value is %s but expected %s" % (proportion_val, expect_mono_range_str),
                )
    ...
    def check_vertical_limits():
        ...
        for glyph_index in range(len(glyf_table.glyphOrder)):
            ...
            if font_props.is_UI_metrics:
                if tests.checkvalue('bounds/glyph/ui_ymax', glyph_index) and ymax is not None and ymax > max_ui_height:
                    warn(
                        "bounds/glyph/ui_ymax",
                        "UI Bounds",
                        "Real yMax for glyph %d (%s) is %d, which is more than "
                        "max ui height %d." % (glyph_index, glyph_name, ymax, max_ui_height),
                        check_test=False,
                    )
                if tests.checkvalue('bounds/glyph/ui_ymin', glyph_index) and ymin is not None and ymin < min_ui_height:
                    warn(
                        "bounds/glyph/ui_ymin",
                        "UI Bounds",
                        "Real yMin for glyph %d (%s) is %d, which is less than "
                        "min ui height %d." % (glyph_index, glyph_name, ymin, min_ui_height),
                        check_test=False,
                    )

            if tests.checkvalue('bounds/glyph/ymax', glyph_index) and ymax is not None and ymax > ascent_limit:
                warn(
                    "bounds/glyph/ymax",
                    "Bounds",
            ...
            if tests.checkvalue('bounds/glyph/ymin', glyph_index) and ymin is not None and ymin < descent_limit:
                warn(
                    "bounds/glyph/ymin",
                    "Bounds",
            ...
            if font_props.is_UI_metrics:
                if font_ymax > max_ui_height:
                    warn(
                        "bounds/font/ui_ymax",
                        "UI Bounds",
                        "Real yMax is %d, but it should be less " "than or equal to %d." % (font_ymax, max_ui_height),
                    )
                ...
                for contour in range(glyph.numberOfContours):
                    ...
                    for point in range(start_point, end_point + 1):
                        if glyph.flags[point] == 1:  # on-curve
                            next_point = point
                            while True:
                                next_point = next_circular_point(next_point, start_point, end_point)
                                if glyph.flags[next_point] == 1:  # on-curve
                                    break

                            curve = curve_between(glyph.coordinates, point, next_point, start_point, end_point)
                ...
                if check_intersection:
                    ...
                    if result:
                        warn(
                            "paths/intersection",
                            "Intersection",
                            "The glyph '%s' has intersecting " "outlines: %s" % (glyph_name, result),
                            check_test=False,
                        )
    ...
    def check_gdef_table(cmap):
        ...
        mark_glyphs = [code for code in cmap if unicode_data.category(code) == 'Mn']
        try:
        ...
        if tests.check('gdef/classdef'):
            names_of_classes = ["default class", "base glyph", "ligature glyph", "mark glyph", "component glyph"]
            if mark_glyphs and not class_def:
                warn(
                    "gdef/classdef/not_present",
                    "Glyph Class",
                ...
                for code in mark_glyphs:
                    ...
                    elif tests.checkvalue('gdef/classdef/combining_mismatch', code) and class_def[glyph] != 3:
                        warn(
                            "gdef/classdef/combining_mismatch",
                            "Glyph Class",
        ...
        if tests.check('gdef/attachlist'):
            ...
            for index, attach_point in enumerate(attach_point_list):
                ...
                if len(set(points)) != len(points):
                    ...
                    warn(
                        "gdef/attachlists/out_of_order",
                        "Attach List",
                        "Points in entry #%d in GDEF.AttachList are not in " "increasing order." % index,
                    )
        ...
        if tests.check('gdef/ligcaretlist'):
            ...
            if ligatures:
                try:
                    lig_caret_list_coverage = font["GDEF"].table.LigCaretList.Coverage
                except (KeyError, AttributeError):
    ...
    def check_shaping(font_file, strs, context, errors):
        ...
        try:
            ...
            temp_file.flush()
            command = ['hb-shape', '--font-file=%s' % font_file, '--text-file=%s' % temp_file.name]
            features = []
            if context != 'isol':
    ...
    def check_gpos_and_gsub_tables():
        ...
        if "GPOS" not in font:
            warn("complex/gpos/missing", "GPOS", "There is no GPOS table in the font.")
        else:
        ...
        if "GSUB" not in font:
            warn("complex/gsub/missing", "GSUB", "There is no GSUB table in the font.")
        else:
    ...
    def check_for_bidi_pairs(cmap):
        ...
        if "GSUB" in font:
            try:
                ...
                warn("bidi", "bidi", "GSUB table with no feature record", is_error=False)
                feature_record = []
            for record in feature_record:
                if record.FeatureTag == "rtlm":  # FIXME
                    for lookup_number in record.Feature.LookupListIndex:
                        lookup = font["GSUB"].table.LookupList.Lookup[lookup_number]
                        ...
                        assert lookup_type == 1, (
                            "Don't know how to handle 'rtlm' features with " "lookup type other than 1."
                        )
        ...
        for code in sorted(cmap):
            if unicode_data.is_private_use(code) or not unicode_data.mirrored(code):
                if cmap[code] in rtlm:
                    warn(
                        "bidi/rtlm_non_mirrored",
                        "Bidi",
                        "The 'rtlm' feature in the font applies to the glyph "
                        "for U+%04X (%s), but it shouldn't, since the "
                        "character is not bidi mirroring." % (code, cmap[code]),
                    )
            ...
            if code in ompl:
                ...
                if mirrored_pair not in cmap:
                    warn(
                        "bidi/ompl_missing_pair",
                        "Bidi",
                        "The character U+%04X (%s) is supported in the font, "
                        "but its bidi mirrored pair U+%04X (%s) is not."
                        % (code, unicode_data.name(code), mirrored_pair, unicode_data.name(mirrored_pair)),
                    )
            else:
                if cmap[code] not in rtlm:
                    warn(
                        ...
                        "not in the OMPL list." % (code, cmap[code]),
                    )
        ...
        if expected_to_be_unhinted:
            for table_name in ['fpgm', 'prep', 'cvt']:
                if table_name in font:
                    warn(
                        "hints/unexpected_tables",
                        "Hints",
                        "The font is supposed to be unhinted, but it has " "a '%s' table." % table_name,
                    )
            ...
            if glyph.numberOfContours > 0:
                ...
                if expected_to_be_unhinted:
                    if check_unexpected_bytecode and bytecode:
                        warn(
                            "hints/unexpected_bytecode",
                            "Hints",
                            "The font is supposed to be unhinted, but " "glyph '%s' has hints." % glyph_name,
                            check_test=False,
                        )
                else:
                    if not bytecode and tests.checkvalue('hints/missing_bytecode', glyph_index):
                        warn(
                            "hints/missing_bytecode",
                            "Hints",
    ...
    def check_explicit_advances():
        ...
        def expect_width(
            codepoint, expected, low_divisor=None, high_divisor=None, label='advances/whitespace', is_error=True
        ):
            ...
            if not high_divisor:
                ...
                if not (low_exp - slop <= adv <= high_exp + slop):
                    ...
                    warn(
                        ...
                        "%d and %d." % (codepoint, glyph_name, glyph_id, adv, low_exp, high_exp),
                        check_test=False,
                        is_error=is_error,
                    )
        ...
        if period_char in cmap:
            ...
            if tests.check('advances/comma_period') and comma_char in cmap:
                expect_width(comma_char, period_width, label='advances/comma_period', is_error=False)
        ...
        if tests.check('advances/spacing_marks'):
            spacing_marks = lint_config.parse_int_ranges("02C8 02CA-02D7 02DE 02DF 02EC 02ED 02EF-02F2 02F4-02FF", True)
            for cp in spacing_marks:
                if cp not in cmap:
                    continue
                if not get_horizontal_advance(cp):
                    warn(
                        "advances/spacing_marks",
                        "Advances",
                        "The spacing mark %s (%04x) should have a non-zero advance." % (unichr(cp), cp),
                    )
            ...
            if check_left_joining and code in joins_to_left:
                if lsb != 0:
                    warn(
                        ...
                        "is %d instead of 0." % (code, unicode_data.name(code), lsb),
                        check_test=False,
                    )
            if check_right_joining and code in joins_to_right:
                ...
                if rsb != -70:
                    warn(
                        ...
                        "is %d instead of -70." % (code, unicode_data.name(code), rsb),
                        check_test=False,
                    )
    ...
    def check_accessiblity(cmap):
        ...
        try:
            ...
            warn("reachable", "Reachability", "Subsetter failure, bad/missing tables?: '%s'" % e)
            return
        ...
        if unreachable_glyphs:
            ...
            if reported_list:
                ...
                warn(
                    "reachable",
                    "Reachability",
                    "The following %d glyphs are unreachable in the font: %s." % (len(reported_glyphs), report_info),
                    check_test=False,
                )
    ...
    if filename_error:
        if filename_error == 'script':
            warn(
                "filename/script",
                "File name",
                "Style %s also needs a script mentioned in the " "file name." % font_props.style,
            )
        elif filename_error == 'name':
            warn(
                "filename/name",
                "File name",
                "File name '%s' does not match the Noto font naming guidelines." % path.basename(font_props.filepath),
            )
    ...
    warn("info", "info", "supported characters: " + printable_unicode_range(cmap.keys()), check_test=False)
...
def main():
    default_config_file = notoconfig.get('lint_config', '[tools]/nototools/data/lint_config.txt')

    parser = argparse.ArgumentParser()
    parser.add_argument("--csv", help="produces csv output to import into a spreadsheet", action="store_true")
    parser.add_argument("--info", help="includes informational messages in the output", action="store_true")
    parser.add_argument(
    ...
    parser.add_argument("--csv_header", help="write header line when generating csv output", action="store_true")
    parser.add_argument(
        "font_files", metavar="font", nargs="*", help="a font file to check, can omit if font_spec is provided"
    )
    parser.add_argument(
        "--font_props_file",
        metavar="file",
    ...
    parser.add_argument("--config", help="extra config spec to process after config file", metavar='lint_spec')
    parser.add_argument("--runlog", help="show tags of run tests", action="store_true")
    parser.add_argument("--skiplog", help="show tags of skipped tests", action="store_true")
    parser.add_argument("--dump_font_props", help="write font props for files", action="store_true")
    parser.add_argument("-nw", "--nowarn", help="suppress warning messages", action="store_true")
    parser.add_argument(
        "-q", "--quiet", help="don't print file names unless there are errors or warnings", action="store_true"
    )
    parser.add_argument(
        "-p", "--phase", help="set noto phase for lint compatibility (default 3)", metavar='phase', type=int, default=3
    )
    parser.add_argument(
        "-v", "--variable", help="do checks appropriate to masters for variable fonts.", action="store_true"
    )
    ...
    if arguments.dump_font_props:
        for font_file_path in arguments.font_files:
            ...
            font_props, filename_error = get_font_properties_with_fallback(font_file_path, phase=arguments.phase)
            if filename_error:
    ...
    if arguments.csv and arguments.csv_header:
        print("Type,Script,Style,Variant,Subfamily,Manufacturer,Category," "Hint Status,File Name,Revision,Issue")
    ...
    if not arguments.csv:
        ...
        if _processed_files > 1:
            if _processed_files_with_errors:
                print(
                    "%d file%s had errors."
                    % (_processed_files_with_errors, '' if _processed_files_with_errors == 1 else 's')
                )
            if _processed_files_with_warnings:
                print(
                    "%d file%s had warnings."
                    % (_processed_files_with_warnings, '' if _processed_files_with_warnings == 1 else 's')
                )
