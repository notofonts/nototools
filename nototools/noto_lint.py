...
WIN_ANSI_CODEPOINTS = (
    '0000-007f 00A0-00ff 20ac 201a 0192 201e 2026 2020 2021 02c6 2030 0160 '
    '2039 0152 017d '
...
def curve_between(
    ...
    if end_at > start_at:
        ...
        return list(coordinates[start_at : end_of_contour + 1]) + list(
            coordinates[start_of_contour : end_at + 1]
        )
...
def curve_has_off_curve_extrema(curve):
    ...
    # the return value of math.atan2 is always in
    # (-pi, +pi], we'll miss the -pi case, resulting in false positives.
    #
    ...
    for sign in [-1, +1]:
        angles = [
            sign * math.pi if math.fabs(angle) == math.pi else angle
            for angle in angles
        ]
...
def out_of_box_size(curve):
    ...
    if len(curve) > 3:
        # If curve has more than 3 points,
        # then it has implicit on-curve points.
...
def interpolate_segment(segment, amount):
    return (
        interpolate(segment[0][0], segment[1][0], amount),
...
def cut_ends(piece, cut_amount):
    if len(piece) == 2:
        return (
            interpolate_segment(piece, cut_amount),
            interpolate_segment(piece, 1 - cut_amount),
        )
    else:
        return bezierTools.splitQuadraticAtT(
            piece[0], piece[1], piece[2], cut_amount, 1 - cut_amount
        )[1]
...
def _build_cmap_dict(filename):
    ...
    return {
        script: frozenset(tool_utils.parse_int_ranges(rd.ranges))
...
def font_properties_from_name(file_path, phase):
    ...
    vendor = (
        ...
        if noto_font.script in ['Khmr', 'Cham', 'Laoo']
        else 'Monotype'
    )
...
def check_font(
    ...
    def warn(
        ...
        def print_file_name():
            ...
            if not _printed_file_name:
                ...
                print(
                    ...
                    % (_cur_file_name, printable_font_versions(font))
                )

        if check_test and not tests.check(test_name):
            return

        interesting_part_of_file_name = ",".join(
            font_props.filepath.split("/")[-2:]
        )
        ...
        if category_name == "info":
            ...
            if not csv_flag and (not quiet or ec or (wc and not nowarn)):
                ...
                if not se:
                    ...
                    print(
                        "Found %s (%s hidden)."
                        % (pluralize_errmsg(ec), "all" if se == ec else se)
                    )
        ...
        if csv_flag:
            ...
            print(
                "%s <%s> %s"
                % (err_type[0], test_name, message.encode('UTF-8'))
            )
        sys.stdout.flush()
    ...
    def _check_unused_names():
        # For now, just a warning, and we don't actually check
        # if other tables use it.
        # Add those checks as we need them.
        # See the GPOS/GSUB checks of name references for an example
        # of how we'd check.
        ...
        for i in names:
            ...
            # names 23 and 24 are for use by CPAL, so it might be considered
            # a mistake if these are present and no CPAL table is present
            ...
            if i >= 256:
                warn(
                    'name/unused',
                    'Name',
                    'Name table has record #%d: "%s"' % (i, names[i]),
                    is_error=False,
                )
    ...
    def check_name_table():
        ...
        if noto_font.is_cjk or noto_phase > 2:
            ...
            _check_idx(
                17, name_data.preferred_subfamily, "preferred subfamily"
            )
            # 18-20 (compatible full, sample text, postscript cid)
        ...
        if (0 <= int(major_version) <= 65535) and (
            0 <= int(minor_version) <= 65535
        ):
            accuracy = len(minor_version)
            font_revision = printable_font_revision(font, accuracy)
            if font_revision != major_version + "." + minor_version:
    ...
    def _check_needed_chars(cmap, char_filter):
        ...
        if char_filter:
            ...
            # TODO(dougfelt): figure out how to make this info available
            #                 without messing up output
            # print(
            # 'filter needed char size: %d -> %d' % (
            # old_needed_size, len(needed_chars)
            # )
            # )
    ...
    def _check_unexpected_chars(cmap, char_filter):
        ...
        if char_filter and unexpected_chars:
            unexpected_chars = set(
                filter(char_filter[1].accept, unexpected_chars)
            )
        if unexpected_chars:
            warn(
                ...
                % (
                    len(unexpected_chars),
                    printable_unicode_range(unexpected_chars),
                ),
                is_error=False,
                check_test=False,
            )
    ...
    def check_cmap_table():
        ...
        if font_props.is_cjk:
            expected_tables.extend(
                [
                    # Adobe says historically some programs used these
                    # to identify the script in the font.
                    # The encodingID is the quickdraw script
        ...
        if tests.check('cmap/non_characters'):
            ...
            if non_characters_in_cmap:
                warn(
                    "cmap/non_characters",
                    "Chars",
                    "There should be no non-characters defined in the font, "
                    "but there are: %s."
        ...
        if tests.check('cmap/disallowed_ascii') and not (
            ...
            if contained_letters:
                warn(
                    ...
                    "Chars",
                    "There should not be ASCII letters in the font, "
                    "but there are: %s."
    ...
    def check_head_tables(cmap):
        ...
        def check_ul_unicode_range():
            ...
            expected_bitmap = font_data.unicoderange_info_to_bitmap(
                expected_info
            )
            ...
            for bucket_index in range(128):
                if difference & (1 << bucket_index):
                    bucket_info = opentype_data.unicoderange_bucket_index_to_info(
                        bucket_index
                    )
                    range_name = opentype_data.unicoderange_bucket_info_name(
                        bucket_info
                    )
                    chars_in_bucket = sum(
                        t[0] for t in expected_info if t[1][2] == bucket_index
                    )
                    size_of_bucket = opentype_data.unicoderange_bucket_info_size(
                        bucket_info
                    )
                    set_unset = (
                        "not be set"
            ...
            # print(
            # "expected %s" % font_data.unicoderange_bitmap_to_string(
            # expected_bitmap
            # )
            # )
            ...
            if font_props.is_UI_metrics:
                if hhea_table.ascent > ui_ascent:
                    warn(
                        "head/hhea/ascent",
                        "Bounds",
                        "Value of ascent in 'hhea' table is %d, "
                        "but should be %d." % (hhea_table.ascent, ui_ascent),
                    )
                if hhea_table.descent < ui_descent:
                    warn(
                        "head/hhea/descent",
                        "Bounds",
                        "Value of descent in 'hhea' table is %d, "
                        "but should be %d." % (hhea_table.descent, ui_descent),
                    )
        ...
        if tests.check('head/os2'):
            if os2_table.fsType != 0:
                warn(
                    "head/os2/fstype",
                    "OS/2",
                    "Value of fsType in the 'OS/2' table is 0x%04X, "
                    "but should be 0." % os2_table.fsType,
                )
            ...
            if os2_table.sTypoDescender != hhea_table.descent:
                warn(
                    "head/os2/descender",
                    "OS/2",
                    "Value of sTypoDescender in 'OS/2' table (%d) "
                    "is different from the value "
                    "of Descent in 'hhea' table (%d), "
                    "but they should be equal."
            ...
            if os2_table.usWinDescent != -hhea_table.descent:
                warn(
                    ...
                    "from the opposite of value of "
                    "Descent in 'hhea' table (%d), "
            ...
            if font_props.is_google and os2_table.achVendID != 'GOOG':
                warn(
                    ...
                    "but should be GOOG." % os2_table.achVendID,
                )
            ...
            if os2_table.fsSelection & OS2_SEL_REGULAR_MASK:
                if os2_table.fsSelection & OS2_SEL_ITALIC_MASK:
                    warn(
                        ...
                        "OS/2",
                        "fsSelection Regular bit is set, "
                        "so the Italic bit should be clear.",
                    )
                if os2_table.fsSelection & OS2_SEL_BOLD_MASK:
                    warn(
                        ...
                        "OS/2",
                        "fsSelection Regular bit is set, "
                        "so the Bold bit should be clear.",
                    )
            ...
            if os2_table.fsSelection & OS2_SEL_USE_TYPO_METRICS_MASK:
                warn(
                    "head/os2/fsselection",
                    "OS/2",
                    "UseTypoMetrics bit in fsSelection is set, "
                    "but should be clear.",
                )
    ...
    def check_vertical_limits():
        ...
        # Build win ansi glyph set.
        # These we compare against usWinAscent/Descent, the rest we compare
        # against sTypoAscender/Descender.
        # Of course, these should be the same, and it's still ok
        # for glyphs to exceed the typo ascender/descender--
        ...
        for glyph_index in range(len(glyf_table.glyphOrder)):
            ...
            if font_props.is_UI_metrics:
                if (
                    tests.checkvalue('bounds/glyph/ui_ymax', glyph_index)
                    and ymax is not None
                    and ymax > max_ui_height
                ):
                    warn(
                        "bounds/glyph/ui_ymax",
                        "UI Bounds",
                        "Real yMax for glyph %d (%s) is %d, which is more "
                        "than max ui height %d."
                        % (glyph_index, glyph_name, ymax, max_ui_height),
                        check_test=False,
                    )
                if (
                    tests.checkvalue('bounds/glyph/ui_ymin', glyph_index)
                    and ymin is not None
                    and ymin < min_ui_height
                ):
                    warn(
                        "bounds/glyph/ui_ymin",
                        "UI Bounds",
                        "Real yMin for glyph %d (%s) is %d, which is less "
                        "than min ui height %d."
                        % (glyph_index, glyph_name, ymin, min_ui_height),
                        check_test=False,
                    )

            if (
                tests.checkvalue('bounds/glyph/ymax', glyph_index)
                and ymax is not None
                and ymax > ascent_limit
            ):
                warn(
                    "bounds/glyph/ymax",
                    "Bounds",
            ...
            if (
                tests.checkvalue('bounds/glyph/ymin', glyph_index)
                and ymin is not None
                and ymin < descent_limit
            ):
                warn(
                    "bounds/glyph/ymin",
                    "Bounds",
            ...
            if font_props.is_UI_metrics:
                ...
                if font_ymin < min_ui_height:
                    warn(
                        "bounds/font/ui_ymin",
                        "UI Bounds",
                        "Real yMin is %d, but it should be greater than or "
                        "equal to %d." % (font_ymin, min_ui_height),
                    )
            else:
                hhea_table = font["hhea"]
                if font_ymax > hhea_table.ascent:
                    warn(
                        "bounds/font/ymax",
                        "Bounds",
                        "Real yMax %d, but it should be less than or equal "
                        "to the value of Ascent in 'hhea' table, "
                ...
                if font_ymin < hhea_table.descent:
                    warn(
                        ...
                        "than or equal to the value of Descent "
                        "in 'hhea' table, which is %d."
                        % (font_ymin, hhea_table.descent),
                    )
                        ...
                        if glyph.flags[point] == 1:  # on-curve
                            ...
                            if out_of_box > 0:
                                warn(
                                    "paths/extrema",
                                    "Extrema",
                                    "The glyph '%s' is missing on-curve "
                                    "extreme points in the segment between "
                                    "point %d=%s and point %d=%s by %f units."
                                    % (
                                        glyph_name,
                                        point,
    ...
    def check_gdef_table(cmap):
        ...
        mark_glyphs = [
            code for code in cmap if unicode_data.category(code) == 'Mn'
        ]
        try:
        ...
        if tests.check('gdef/classdef'):
            ...
            if mark_glyphs and not class_def:
                warn(
                    ...
                    "font, while there are non-spacing combining characters: "
                    "%s." % printable_unicode_range(mark_glyphs),
                    is_error=False,
                )
                    ...
                    if glyph not in class_def:
                        if tests.checkvalue('gdef/classdef/unlisted', code):
                            warn(
                                ...
                                "Glyph Class",
                                "Glyph %s (U+%04X %s) is a combining mark, "
                                "but is not assigned a value "
                                "in the GDEF/GlyphClassDef table."
                        ...
                        tests.checkvalue(
                            'gdef/classdef/combining_mismatch', code
                        )
                        and class_def[glyph] != 3
                    ):
                        warn(
                            "gdef/classdef/combining_mismatch",
                            "Glyph Class",
                            "Glyph %s (U+%04X %s) is a combining mark, but is "
                            "defined as class %d (%s) "
                            "in the GDEF/GlyphClassDef table."
                            % (
                                glyph,
                                code,
            ...
            if class_def and not is_indic:
                for code in cmap:
                    ...
                    if glyph in class_def:
                        ...
                        if (
                            tests.checkvalue(
                                'gdef/classdef/not_combining_mismatch', code
                            )
                            and klass == 3
        ...
        if tests.check('gdef/ligcaretlist'):
            ...
            if ligatures:
                try:
                    lig_caret_list_coverage = font[
                        "GDEF"
                    ].table.LigCaretList.Coverage
                except (KeyError, AttributeError):
                ...
                if not lig_caret_list_coverage:
                    if not is_indic:
                        warn(
                            ...
                            "Ligature Class",
                            "There is no LigCaretList data in the GDEF table, "
                            "but there are ligatures defined in GDEF: %s."
                            % ", ".join(ligatures),
                        )
                else:
                    if set(lig_caret_list_coverage.glyphs) - set(ligatures):
                        warn(
                            ...
                            "Ligature Class",
                            "Some glyphs are defined to have ligature carets "
                            "in GDEF table, "
                            "but are not defined as ligatures in the "
                            "table: %s."
                            % ", ".join(
                                sorted(
                        ...
                        if not is_indic:
                            warn(
                                ...
                                "Ligature Class",
                                "Some glyphs are defined as ligatures "
                                "in the GDEF table, "
                                "but don't have ligature carets: %s."
                                % ", ".join(
                                    sorted(
                                        set(ligatures)
                                        - set(lig_caret_list_coverage.glyphs)
            ...
            if isinstance(params, otTables.FeatureParamsStylisticSet):
                ...
                if params.UINameID not in name_id_set:
                    warn(
                        ...
                        GSUB_OR_GPOS,
                        "Feature index %s (%s) has UINameID %d "
                        "but it is not in the name table"
    ...
    def check_for_bidi_pairs(cmap):
        ...
        if "GSUB" in font:
            try:
                ...
                warn(
                    "bidi",
                    "bidi",
                    "GSUB table with no feature record",
                    is_error=False,
                )
                feature_record = []
        ...
        for code in sorted(cmap):
            if unicode_data.is_private_use(code) or not unicode_data.mirrored(
                code
            ):
                ...
                if mirrored_pair not in cmap:
                    warn(
                        ...
                        % (
                            ...
                            unicode_data.name(code),
                            mirrored_pair,
                            unicode_data.name(mirrored_pair),
                        ),
                    )
            else:
        ...
        for glyph_index in range(len(glyf_table.glyphOrder)):
            ...
            if glyph.numberOfContours > 0:
                ...
                if expected_to_be_unhinted:
                    ...
                    if not bytecode and tests.checkvalue(
                        'hints/missing_bytecode', glyph_index
                    ):
                        warn(
                            "hints/missing_bytecode",
                            "Hints",
                            "The font is supposed to be hinted, but "
                            "glyph '%s' (%d) doesn't have hints."
    ...
    def check_explicit_advances():
        ...
        def get_horizontal_advance(codepoint):
            return font_data.get_glyph_horizontal_advance(
                font, cmap[codepoint]
            )
            ...
            if not high_divisor:
                ...
                if abs(adv - exp) > slop:
                    ...
                    warn(
                        label,
                        "Advances",
                        "The advance of U+%04x (%s, glyph %d) is %d, "
                        "but expected %d."
                ...
                # note name switch, since the higher divisor
        ...
        if tests.check('advances/whitespace'):
            if font_props.is_mono:
                ...
                # but microsoft (below) says French uses 1/8 em.
            ...
            for cp in spacing_marks:
                ...
                if not get_horizontal_advance(cp):
                    warn(
                        ...
                        "Advances",
                        "The spacing mark %s (%04x) "
                        "should have a non-zero advance." % (unichr(cp), cp),
                    )
    ...
    def check_accessiblity(cmap):
        ...
        if unreachable_glyphs:
            ...
            if reported_list:
                report_info = ', '.join(
                    '%s (%d)' % t for t in sorted(reported_list)
                )
                warn(
                    "reachable",
                    "Reachability",
    ...
    is_indic = font_props.script in {
        ...
        version=printable_font_revision(
            font, accuracy=3 if font_props.vendor == 'Adobe' else 2
        ),
    )
    ...
    if filename_error:
        if filename_error == 'script':
            ...
            warn(
                "filename/name",
                "File name",
                "File name '%s' does not match "
                "the Noto font naming guidelines."
    ...
    # since the subsetter may change the font
...
def main():
    ...
    parser.add_argument(
        "--font_props_file",
        metavar="file",
        help="file containing json describing font file paths "
        "and expected properties",
    )
    parser.add_argument(
        "--config_file",
        help="location of config file (default %s, 'None' for none)"
        % default_config_file,
        metavar='file',
    ...
    parser.add_argument(
        "--runlog", help="show tags of run tests", action="store_true"
    )
    parser.add_argument(
        "--skiplog", help="show tags of skipped tests", action="store_true"
    )
    parser.add_argument(
        "--dump_font_props",
    ...
    if arguments.csv and arguments.csv_header:
        print(
            "Type,Script,Style,Variant,Subfamily,Manufacturer,Category,"
