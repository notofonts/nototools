...
def print_contextual_substitution(
    backtrack, center, lookahead, subs, reverse=False
):
...
def dump_gsub_subtable(lookup_type, subtable):
    ...
    if lookup_type == 1:
        ...
        print_indented(
            'type=%d format=%d not supported yet'
...
def dump_bases(glyphs, records, printable_function):
    ...
    for glyph in glyphs:
        record = records[index]
        print_indented(
            '%s: %s' % (glyph, printable_function(record)), indents=2
        )
        index += 1
...
def dump_gpos_subtable(lookup_type, subtable):
    ...
    if lookup_type == 1 and subtable.Format == 1:
        ...
        for index1, class1_record in enumerate(subtable.Class1Record):
            ...
            for index2, class2_record in enumerate(class1_record.Class2Record):
                ...
                if class2_record.Value2 is None:
                    value_record1 = printable_value_record(
        ...
        dump_marks(
            subtable.Mark1Coverage.glyphs, subtable.Mark1Array.MarkRecord
        )
