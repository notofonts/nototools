...
def internal_font_name(font):
    ...
    for record in name_table.names:
        identifier = (record.nameID, record.platformID, record.platEncID, record.langID)
...
def print_contextual_substitution(backtrack, center, lookahead, subs, reverse=False):
    ...
    if subs:
        if isinstance(subs[0], str):
            ...
            records_added = 0
            for record in subs:
                output_list.insert(record.SequenceIndex + 1 + records_added, 'lookup %d' % record.LookupListIndex)
                records_added += 1
...
def dump_gsub_subtable(lookup_type, subtable):
    ...
    if lookup_type == 1:
        ...
        for key in sorted(subtable.alternates.keys()):
            print_indented('sub %s from %s;' % (key, printable_glyph_class(subtable.alternates[key])))

    elif lookup_type == 4:
        for key in sorted(subtable.ligatures.keys()):
            for ligature in subtable.ligatures[key]:
                print_indented('sub %s by %s;' % (printable_glyph_list([key] + ligature.Component), ligature.LigGlyph))
        ...
        for index in range(subtable.SubRuleSetCount):
            ...
            for subrule in subtable.SubRuleSet[index].SubRule:
                ...
                print_contextual_substitution([], whole_input, [], subrule.SubstLookupRecord)

    elif lookup_type == 5 and subtable.Format == 2:
        print_indented('type=%d format=%d not supported yet' % (lookup_type, subtable.Format))
    ...
    #                print(index, sub_class)
        ...
        print_contextual_substitution([], subtable.Coverage, [], subtable.SubstLookupRecord)
...
def printable_device(device):
    ...
    for index in range(len(device.DeltaValue)):
        output_list.append('%d %d' % (device.StartSize + index, device.DeltaValue[index]))
...
def printable_anchor(anchor):
    ...
    if anchor.Format == 1:
        ...
        return '<anchor %d %d contourpoint %d>' % (anchor.XCoordinate, anchor.YCoordinate, anchor.AnchorPoint)
...
def dump_marks(glyphs, records):
    ...
    for glyph in glyphs:
        ...
        print_indented('%s: class=%d %s' % (glyph, record.Class, printable_anchor(record.MarkAnchor)), indents=2)
        index += 1
...
def dump_gpos_subtable(lookup_type, subtable):
    ...
    if lookup_type == 1 and subtable.Format == 1:
        print_indented(
            'position %s %s;'
            % (printable_glyph_class(subtable.Coverage.glyphs), printable_value_record(subtable.Value))
        )

    elif lookup_type == 1 and subtable.Format == 2:
        for index, value_record in enumerate(subtable.Value):
            print_indented('position %s %s;' % (subtable.Coverage.glyphs[index], printable_value_record(value_record)))
        ...
        for index, pair_set in enumerate(subtable.PairSet):
            ...
            for pair_value_record in pair_set.PairValueRecord:
                if pair_value_record.Value2 is None:
                    print_indented(
                        'position %s %s %s;'
                        % (first_glyph, pair_value_record.SecondGlyph, printable_value_record(pair_value_record.Value1))
                    )
                else:
                    print_indented(
                        'position %s %s %s %s;'
        ...
        for index1, class1_record in enumerate(subtable.Class1Record):
            ...
            for index2, class2_record in enumerate(class1_record.Class2Record):
                ...
                if class2_record.Value2 is None:
                    ...
                    print_indented(
                        'position %s %s %s;'
                        % (printable_glyph_class(class1), printable_glyph_class(class2), value_record1)
                    )
                else:
                    print_indented(
                        'position %s %s %s %s;'
        ...
        dump_bases(
            ...
            lambda record: ' '.join([printable_anchor(anchor) for anchor in record.BaseAnchor]),
        )
        ...
        print_indented('Liga:')
        dump_bases(subtable.LigatureCoverage.glyphs, subtable.LigatureArray.LigatureAttach, printable_ligature_attach)

    elif lookup_type == 6:
        print_indented('Mark1:')
        dump_marks(subtable.Mark1Coverage.glyphs, subtable.Mark1Array.MarkRecord)

        print_indented('Mark2:')
        dump_bases(
            subtable.Mark2Coverage.glyphs,
            subtable.Mark2Array.Mark2Record,
            lambda record: ' '.join([printable_anchor(anchor) for anchor in record.Mark2Anchor]),
        )

    elif lookup_type == 8 and subtable.Format == 3:
        print_contextual_substitution(
            subtable.BacktrackCoverage, subtable.InputCoverage, subtable.LookAheadCoverage, subtable.PosLookupRecord
        )
    else:
        print_indented('type=%d format=%d not supported yet' % (lookup_type, subtable.Format))
...
def dump_script_record(script_record):
    ...
    for script in script_record:
        ...
        for lang_sys_record in script.Script.LangSysRecord:
            dump_lang_sys(script_tag, lang_sys_record.LangSysTag, lang_sys_record.LangSys)
...
def dump_lookup_list(lookup_list, table_name):
    ...
    for index in range(len(lookup_list)):
        ...
        print('lookup %d { # type=%d flag=0x%X' % (index, lookup.LookupType, lookup.LookupFlag))
