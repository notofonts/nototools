...
def dump_gsub_subtable(lookup_type, subtable):
    ...
    if lookup_type == 1:
        ...
        for key in sorted(subtable.alternates.keys()):
            print_indented(
                "sub %s from %s;"
                % (key, printable_glyph_class(subtable.alternates[key]))
            )
        ...
        print_indented(
            "type=%d format=%d not supported yet" % (lookup_type, subtable.Format)
        )
...
def dump_gpos_subtable(lookup_type, subtable):
    ...
    if lookup_type == 1 and subtable.Format == 1:
        ...
        dump_bases(
            ...
            lambda record: " ".join(
                [printable_anchor(anchor) for anchor in record.BaseAnchor]
            ),
        )
        ...
        dump_bases(
            ...
            lambda record: " ".join(
                [printable_anchor(anchor) for anchor in record.Mark2Anchor]
            ),
        )
...
def dump_script_record(script_record):
    ...
    for script in script_record:
        ...
        for lang_sys_record in script.Script.LangSysRecord:
            dump_lang_sys(
                script_tag, lang_sys_record.LangSysTag, lang_sys_record.LangSys
            )
