...
def check_families(family_map):
    ...
    for family_id, family in sorted(family_map.items()):
        ...
        if (
            ...
            # Let's not consider this an error for now.
            # Just drop the members with the higher number of fonts,
            # assuming it's a superset of the fonts in the smaller set,
            # so that the fonts we provide and display are available to
            # all users.
            # This means website users will not be able to get these fonts
            # via the website.
            #
            ...
            # The namedtuples are immutable,
            # so we need to break them apart and reform them
            name = family.name
    ...
    for family_id in sorted(family_id_to_lang_scrs):
        ...
        for lang_scr in sorted(family_id_to_lang_scrs[family_id]):
            ...
            for info in sample_infos:
                ...
                if full_key in tested_keys:
                    if full_key in failed_keys:
                        print(
                            'family %s already rejected sample %s (lang %s)'
            ...
            if not sample_key_for_lang:
                print(
                    '%s has no sample to display in %s' % (lang_scr, family_id)
                )
            else:
        ...
        if not lang_scr_to_sample_key:
            print(
                ...
                % (
                    family_id,
                    ', '.join(sorted(family_id_to_lang_scrs[family_id])),
                )
            )
        else:
            print(
                '%s has samples for %s langs'
...
def get_named_lang_scrs(family_id_to_lang_scr_to_sample_key):
    ...
    for family_id in family_id_to_lang_scr_to_sample_key:
        lang_scrs = [
            l
            for l in family_id_to_lang_scr_to_sample_key[family_id].keys()
...
def get_attribution(lang_scr_typ):
    if not ATTRIBUTION_DATA:
        attribution_path = path.join(
            TOOLS_DIR, 'sample_texts', 'attributions.txt'
        )
...
def get_family_id_to_default_lang_scr(family_id_to_lang_scrs, families):
  ...
  """
    ...
    for family_id, lang_scrs in family_id_to_lang_scrs.items():
        ...
        if lang_scr not in lang_scrs:
            print(
                'default lang_scr \'%s\' not listed for family %s %s'
...
def check_debug(debug):
    ...
    for key in debug:
        if key not in _DEBUG_KEYS:
            print(
                'Bad debug key(s) found.  Keys are:\n  %s'
...
class WebGen(object):
    def __init__(
        ...
        self.no_build = no_build or (
            no_zips and no_images and no_css and no_data
        )
    ...
    def clean_target_dir(self):
        if path.exists(self.target):
            print(
                'Removing the old website directory from %s...' % self.target
            )
    ...
    def build_family_zips(self, key, family):
        readme_key = self.get_readme_key_for_filepath(
            family.rep_member.filepath
        )
    ...
    def build_family_css(self, key, family):
        fonts = [
            m
            for m in (family.hinted_members or family.unhinted_members)
            if not m.is_UI
        ]
        fonts.sort(
            key=lambda f: (
        ...
        with open(css_path, 'w') as css_file:
            for font in fonts:
                ...
                max_font_size = max(
                    max_font_size, os.stat(font.filepath).st_size
                )
                # Make it possible to access Mono cjk variants
                # and those with irregular css values
                # by assigning them other names.
                ...
                if weight % 100 != 0:
                    ...
                    # prevent auto-bolding of this font
                    # by describing it as bold
                    weight = 700
    ...
    def build_data_json(
        ...
        # Sort families by English name,
        # except Noto Sans/Serif/Mono come first.
        initial_ids = [
            'sans-lgc',
            'serif-lgc',
            'sans-lgc-display',
        ...
        for k in sorted_ids:
            ...
            if family.rep_member.is_cjk:
                ...
                num_fonts = sum(
                    1
                    for f in (family.hinted_members or family.unhinted_members)
                    if not f.is_UI
                )
                if num_fonts not in [1, 2, 3, 4, 9, 12, 36, 72]:
                    print(
                        'family %s (%s) has %d fonts'
                        % (k, family.name, num_fonts)
                    )
                    print(
                        '\n'.join(
                            f.filepath
        ...
        for (
            family_id,
            lang_scrs,
        ) in family_id_to_lang_scr_to_sample_key.items():
            for lang_scr in lang_scrs:
        ...
        # listed in the UI
        lang_scrs = [
            l for l in lang_scr_to_family_ids if not l.startswith('und-')
        ]

        langs_obj = collections.OrderedDict()
        # sort by english name
        for lang_scr in sorted(
            lang_scrs, key=lambda l: lang_data.lang_script_to_names(l)[0]
        ):
    ...
    def build_family_json(
        ...
        if lang_scrs_map:
            ...
            lang_scrs = [
                l for l in lang_scrs_map.keys() if not l.startswith('und-')
            ]
    ...
    def build_family_images(
        ...
        for font in displayed_members:
            ...
            if is_cjk and family.name.find('Serif') < 0:
                # The sans and serif cjk's are named differently,
                # and it confuses fontconfig.
                # Sans includes 'Regular' and 'Bold' in the standard
                # font names, but serif doesn't.
                # Fontconfig registers two names for these in the sans
                # (with and without the weight), but only
                # the name without weight for the serif.
                # So if you ask pango/cairo for the serif font and include
                # the weight name, it fails and falls back to some
                # non-noto font.
            ...
            create_image.create_img(
                ...
                rtl=is_rtl,
                width=685,
                # text is coming out bigger than we expect,
                # perhaps this is why?
    ...
    def build_images(
        ...
        for family_id in sorted(family_id_to_lang_scr_to_sample_key):
            ...
            # name them based on the language.
            # But most of the samples with the same font and text will
            # be the same, because the fonts generally
    ...
    def build_ttc_zips(self):
        ...
        # the copy.
        # The Serif ttc is so big github won't let us push it.
        # In fact, I can't even commit it to my repo
        # because then I can't push anything.
        # So the serif ttc might not be here.
        # We want to provide it, but we don't have it in git so the README
        # doesn't apply.
        # Not sure what to do about this, for now don't include
        # a README for it.
        # There's no git repo for people to trace this file back to.
            ...
            for subset in ['KR', 'JP', 'SC', 'TC', 'HK']:
                ...
                if not filenames:
                    raise Exception(
                        ...
                        % (
                            CJK_DIR,
                            family_pat,  # TODO: family_pat is not defined!  # noqa:F821
                        )
                    )
                ...
                print("Wrote " + zip_path)
                print(
                    'Compressed from {0:,}B to {1:,}B.'.format(
                        oldsize, newsize
                    )
                )
    ...
    def generate(self):
            ...
            for i, (family_id, family) in enumerate(sorted(families.items())):
                print(
                    ...
                    % (
                        ...
                        family_id,
                        family.name,
                        noto_fonts.get_family_filename(family),
                    )
                )
                if family.hinted_members:
                    print(
                        ...
                        % ', '.join(
                            sorted(
                                [
                                    ...
                                    for m in family.hinted_members
                                ]
                            )
                        )
                    )
                    ...
                    print(
                        ...
                        % ', '.join(
                            sorted(
                                [
                                    path.basename(m.filepath)
                                    for m in family.unhinted_members
                                ]
                            )
                        )
                    )
        ...
        if 'script_to_family_ids' in self.debug:
            ...
            for i, (script, family_ids) in enumerate(
                ...
                print(
                    '%2d] %s: %s' % (i, script, ', '.join(sorted(family_ids)))
                )
        ...
        if 'lang_scr_to_sample_infos' in self.debug:
            ...
            for lang_scr, info_list in sorted(
                lang_scr_to_sample_infos.items()
            ):
        ...
        if 'family_id_to_lang_scr_to_sample_key' in self.debug:
            ...
            for i, (family_id, lang_scr_to_sample_key) in enumerate(
                ...
                print(
                    '%2d] %s (%d):'
                    % (i, family_id, len(lang_scr_to_sample_key))
                )
        ...
        for family in sorted(families.values()):
            ...
            for lang_scr in sorted(lang_scr_to_sample_key):
                ...
                if sample_key not in sample_key_to_info:
                    error_list.append(
                        'no sample for sample key: %s' % sample_key
                    )
        ...
        if self.no_zips and self.no_data:
            ...
            # build outputs not used by the json but linked
...
def main():
    ...
    parser.add_argument(
        '-c', '--clean', help='clean target directory', action='store_true'
    )
    parser.add_argument(
        '-d',
        '--dest',
        help='target dir, default %s' % default_dest,
    ...
    parser.add_argument(
        '-nz', '--no_zips', help='skip zip generation', action='store_true'
    )
    parser.add_argument(
        '-ni', '--no_images', help='skip image generation', action='store_true'
    )
    parser.add_argument(
        '-nd', '--no_data', help='skip data generation', action='store_true'
    )
    parser.add_argument(
        '-nc', '--no_css', help='skip css generation', action='store_true'
    )
    parser.add_argument(
        '-n',
        '--no_build',
    ...
    parser.add_argument(
        '--debug', help='types of information to dump during build', nargs='*'
    )
