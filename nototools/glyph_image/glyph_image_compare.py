...
def write_compare_data(gic_data, fd):
  def write_font_data(label, fdata):
    ...
    for k in GlyphImageFontData._fields:
      print >> fd, '> %s: %s' % (k, getattr(fdata, k))
    print >> fd, ']'

  def write_glyph_data(label, gdata):
    print >> fd, '> %s: %d' % (label, len(gdata))
    print >> fd, '# index advance cp name'
    for i, (adv, cp, g) in enumerate(gdata):
      print >> fd, '%d;%d;%s;%s' % (
          i, adv, '%04x' % cp if cp >= 0 else '', g or '')

  def write_pair_data(label, pdata):
    print >> fd, '> %s:' % label
    print >> fd, '> max_frame: %d %d %d %d' % pdata.max_frame
    print >> fd, '> pairs: %d' % len(pdata.pair_data)
    print >> fd, '# image_name base target similarity (pct)'
    for name, base, target, similarity in pdata.pair_data:
      base_str = '%d' % base if base >= 0 else ''
      target_str = '%d' % target if target >= 0 else ''
      print >> fd, '%s;%s;%s;%s' % (name, base_str, target_str, similarity)

  time = datetime.now()
  print >> fd, '# %s' % time.strftime('%Y-%m-%d %H:%M:%S')
...
def write_compare_data_to_dir(
    gic_data, output_dir, name='compare_data.txt'):
...
def compare_collections(
    base_collection, target_collection, pair_data, output_dir):
  ...
  if base_header.name != target_header.name:
    print 'base name is "%s" but target name is "%s"' % (
  ...
  if base_header.size != target_header.size:
    print 'base font size is %d but target font size is %d' % (
  ...
  for name, base_ix, target_ix in named_pairs:
    ...
    frame = glyph_image.pad_frame(
        glyph_image.union_frames([frame, tall_frame]), 5)
...
def compare_files(base_file, target_file, image_dir):
  ...
  if not path.isfile(target_file):
    raise Exception(
        'target file %s does not exist' % target_file)
  ...
  return compare_collections(
      base_collection, target_collection, None, image_dir)


def main():
  parser = argparse.ArgumentParser()
  parser.add_argument(
    '-b', '--base', help='base glyph image file', metavar='file',
    required=True)
  parser.add_argument(
    '-t', '--target', help='target glyph image file', metavar='file',
    required=True)
  parser.add_argument(
    '-o', '--output_dir', help='directory to write output into', metavar='dir',
    required=True)
