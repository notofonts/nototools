...
PairInfo = collections.namedtuple(
    'PairInfo', 'base_path base_hash target_path target_hash cp_pairs '
...
class HungarianMatcher(object):
  ...
  def _setup(self):
    ...
    for r in xrange(rows):
      ...
      for m in xrange(n, lim):
        data[m] -= min_val
      n = lim

    # Step 2. Scan each row for a zero in a column with no starred zero, and
    # star it.
    if self.dbg:
      print 'step 2'
  ...
  def _3_cover_starred_zeros(self):
    # Step 3.  Cover each column with a starred zero.  If after this all columns
    # are covered, or more columns than rows are covered, we're done.  Otherwise
    # go to step 4 prime_uncovered_zeros.
  ...
  def _4_prime_uncovered_zeros(self):
    # Step 4. Find an uncovered zero and prime it.  If there is no unstarred
    # zero in its row, stop, and go to step 5 build prime star path and adjust.
    # Else cover this row and uncover this column. Continue until there are no
    # uncovered zeros, then go to step 6
  ...
  def _6_find_smallest_uncovered_and_adjust(self):
    # Step 6.  Find the smallest uncovered value.  Add it to each covered row
...
def _get_image_diff_pairs(
  ...
  log = False
  if log:
    print 'get image diff pairs, %d base, %d target' % (
  ...
  diff_pool = {}
  exact_matches = []
  for base_ix, base_fp in sorted(base_ix_to_fingerprint.iteritems()):
    if log:
    ...
    if not target_ix_to_fingerprint:
      ...
      break
    for target_ix, target_fp in sorted(target_ix_to_fingerprint.iteritems()):
  ...
  # of the remaining, we have the best matches for the base, but not for target
  ...
  # other col or row.
  reciprocal_pairs = []
  for base_ix, (diff, t) in sorted(best_base_diffs.iteritems()):
  ...
  if base_ix_to_fingerprint and target_ix_to_fingerprint:
    ...
    for (b, t), d in diff_pool.iteritems():
  ...
  if log and missing_rp_count:
    ...
    print 'missing %d of %d reciprocal pair%s' % (
...
def get_collection_pairs(base_collection, target_collection):
  ...
  return PairInfo(
      base_font_path, base_hash, target_font_path, target_hash, cp_pairs,
      pri_pairs, alt_base_pairs, alt_target_pairs)
...
def main():
  ...
  parser.add_argument(
      '-b', '--base', help='base glyph image data file', metavar='file',
      required=True)
  parser.add_argument(
      '-t', '--target', help='target glyph image data file', metavar='file',
      required=True)
  parser.add_argument(
