...
def write_file_header(file_header, fd):
  for k in FileHeader._fields:
    print >> fd, '> %s: %s' % (k, getattr(file_header, k))
...
def write_glyph_image(im, glyph_header_only, fd):
  ...
  print >> fd, '> glyph: %d;%s;%d %d %d %d' % (
      (im.index, adv_string) + im.frame)
  if not glyph_header_only:
    print >> fd, im.image_str()
...
def frame_contains(fr, x, y):
  return x >= fr.l and y >= fr.t and x < (fr.l + fr.w) and y < (fr.t + fr.h)


def frame_is_empty(frame):
  return frame == None or frame.w <= 0 or frame.h <= 0
