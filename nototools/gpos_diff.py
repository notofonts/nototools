...
class GposDiffFinder:
    ...
    def __init__(self, file_a, file_b, error_bound, output_lines=6):
        ...
        subprocess.call(['ttxn', '-q', '-t', 'GPOS', '-o', ttxn_file_a.name, '-f', file_a])
        subprocess.call(['ttxn', '-q', '-t', 'GPOS', '-o', ttxn_file_b.name, '-f', file_b])
    ...
    def find_kerning_diffs(self):
        ...
        # 2. Left-side glyph name
        # 3. Right-side glyph name
        mismatched.sort(key=lambda t: (-sum(abs(v1 - v2) for v1, v2 in zip(t[1][0], t[1][1])), t[0][0], t[0][1]))
    ...
    def find_mark_class_diffs(self):
        ...
        unmatched = {}
        mismatched = {}
        rx = re.compile('mark \[([\w\d\s@_.]+)\] <anchor (-?\d+) (-?\d+)> ' '(@[\w\d_.]+);')
        ...
        for (sign, member, mark_class), (x, y) in unmatched[: self.out_lines]:
            res.append('%s mark [%s] <anchor %d %d> %s;' % (sign, member, x, y, mark_class))
        res.append('')
        ...
        # 3. Mark class
        mismatched.sort(
            key=lambda t: (-(abs(t[1][0][0] - t[1][1][0]) + abs(t[1][0][1] - t[1][1][1])), t[0][0], t[0][1])
        )
        for (member, cls), ((x1, y1), (x2, y2)) in mismatched[: self.out_lines]:
            if abs(x1 - x2) > self.err or abs(y1 - y2) > self.err:
                res.append('%s %s <%d %d> vs <%d %d>' % (member, cls, x1, y1, x2, y2))
        res.append('')
        return '\n'.join(res)
    ...
    def find_positioning_diffs(self, mark_type='base'):
        ...
        unmatched = {}
        mismatched = {}
        rx = re.compile('pos %s \[([\w\d\s@_.]+)\]\s+<anchor (-?\d+) (-?\d+)> ' 'mark (@?[\w\d_.]+);' % mark_type)
        self._parse_anchor_info(rx, '-', self.text_a, unmatched, mismatched)
        self._parse_anchor_info(rx, '+', self.text_b, unmatched, mismatched)

        res = ['%d differences in mark-to-%s positioning rule coverage' % (len(unmatched), mark_type)]
        unmatched = list(unmatched.items())
        # Sort order: same as 'mark class definitions'
        unmatched.sort(key=lambda t: (t[0][1], t[0][2]))
        for (sign, member, mark_class), (x, y) in unmatched[: self.out_lines]:
            res.append('%s pos %s [%s] <anchor %d %d> mark %s;' % (sign, mark_type, member, x, y, mark_class))
        res.append('')

        res.append('%d differences in mark-to-%s positioning rule values' % (len(mismatched), mark_type))
        mismatched = list(mismatched.items())
        ...
        for rule in rx.findall(text):
            ...
            for left_glyph in left:
                for right_glyph in right:
                    ...
                    key_match = (self._reverse_sign(sign), left_glyph, right_glyph)
    ...
    def _organize_kerning_diffs(self, unmatched, mismatched):
        ...
        for key in keys:
            ...
            if key_match in unmatched and unmatched[key] and unmatched[key_match]:
                if sign == '+':
                    key, key_match = key_match, key
                mismatched[left, right] = (unmatched.pop(key), unmatched.pop(key_match))
            ...
            for member in members.split():
                ...
                if key_match in unmatched:
                    if unmatched[key_match] != val:
                        mismatched[member, mark_class] = (unmatched[key_match], val)
