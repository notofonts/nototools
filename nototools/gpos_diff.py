...
class GposDiffFinder:
    ...
    def find_kerning_diffs(self):
        ...
        unmatched.sort(
            key=lambda t: (-max(abs(v) for v in t[1]), t[0][1], t[0][2])
        )
    ...
    def find_mark_class_diffs(self):
        ...
        for (member, cls), ((x1, y1), (x2, y2)) in mismatched[
            : self.out_lines
        ]:
            if abs(x1 - x2) > self.err or abs(y1 - y2) > self.err:
                res.append(
                    '%s %s <%d %d> vs <%d %d>' % (member, cls, x1, y1, x2, y2)
                )
        res.append('')
        return '\n'.join(res)
    ...
    def _organize_kerning_diffs(self, unmatched, mismatched):
        ...
        for key in keys:
            ...
            if (
                key_match in unmatched
                and unmatched[key]
                and unmatched[key_match]
            ):
                if sign == '+':
                    key, key_match = key_match, key
                mismatched[left, right] = (
                    unmatched.pop(key),
                    unmatched.pop(key_match),
                )
    ...
    def _parse_anchor_info(self, rx, sign, text, unmatched, mismatched):
        ...
        for members, x, y, mark_class in rx.findall(text):
            # hack to get around unexpected class naming differences
            # (ttxn bug?)
