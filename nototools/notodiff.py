...
def _shape(
    path_a, path_b, stats, diff_type, font_size, render_path, diff_threshold=0
):
...
def _gpos(path_a, path_b, error_bound, out_lines, print_font=False):
    ...
    diff_finder = gpos_diff.GposDiffFinder(
        path_a, path_b, error_bound, out_lines
    )
        ...
        if stats:
            print(
                shape_diff.ShapeDiffFinder.dump(
                    ...
                    args.out_lines,
                    include_vals=(
                        args.diff_type in ('area', 'area-shape-product')
                    ),
