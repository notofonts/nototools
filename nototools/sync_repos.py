...
def main():
    ...
    parser.add_argument(
        "-m", "--master", help="use master branch for all repos", action="store_true"
    )
    parser.add_argument(
        "-v", "--verbose", help="report tags chosen on success", action="store_true"
    )
    parser.add_argument(
        "-n",
        "--dry_run",
    ...
    if args.master:
        ...
        result = noto_checkout(
            fonts_tag=args.fonts_tag,
            emoji_tag=args.emoji_tag,
            cjk_tag=args.cjk_tag,
