...
class TestGlyphAreas(unittest.TestCase):
    ...
    Must have the following attributes:
        main/instance_glyph_sets: Tuples each containing a list of font
            filenames and a list of glyph sets corresponding to those fonts.
        main/instance_glyphs_to_test: Lists of glyph names.
        main/instance_weights_to_test: Lists of weights as strings, should
            have exactly two main weights and >1 instance weight.
        ...
        allowlist: List containing names of glyphs to not test.
    ...
    main/instance per weight is included from the given glyph sets.
    """
    ...
    def setUp(self):
        ...
        self.unchanged = set()
        main_a, main_b = self.getGlyphSets(
            self.main_glyph_sets, self.main_weights_to_test
        )

        pen_a = GlyphAreaPen(main_a)
        pen_b = GlyphAreaPen(main_b)
        for name in self.main_glyphs_to_test:
            if name in self.allowlist:
                continue
            main_a[name].draw(pen_a)
            area_a = pen_a.pop()
            main_b[name].draw(pen_b)
            area_b = pen_b.pop()
            if area_a == area_b:
                if area_a:
